...
PATTERNS = [
    ...
    (
        "API key",
        r"(?i)\b(?:api[_-]?key|secret[_-]?key|access[_-]?token)\s*[:=]\s*['\"][^'\"]{8,}['\"]",
        "High",
    ),
    (
        "Password",
...
def _redact(value):
    return re.sub(
        r"(?i)(password|passwd|pwd|api[_-]?key|secret[_-]?key|token|authorization)\s*[:=]\s*(['\"])[^'\"]*\2",
        r"\1=\2[REDACTED]\2",
        value or "",
    )
...
def analyze_secret_exposure(repo_path, files):
                            ...
                            findings.append(
                                {
                                    ...
                                    "type": kind,
                                    "severity": severity,
                                    "message": "Potential secret detected. The source value is redacted.",
                                    "code": _redact(stripped),
                                }
                            )
                            break
    ...
    return {
        ...
        "sensitive_file_count": len(set(sensitive_files)),
        "findings": sorted(
            unique.values(),
            key=lambda x: (x["file"], x["line"]),
        ),
        "secret_finding_count": len(unique),
    }