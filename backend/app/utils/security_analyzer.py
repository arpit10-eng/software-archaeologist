...
def _redact(line):
    return re.sub(
        r"(?i)(password|passwd|pwd|api[_-]?key|secret[_-]?key|token|authorization)\s*([:=])\s*(['\"])[^'\"]*\3",
        r"\1\2\3[REDACTED]\3",
        line,
    )
...
def detect_database_credentials(line):
    match = re.search(
        r"(database_url|db_url|database_uri|db_uri)\s*=\s*['\"]([^'\"]+)['\"]",
        line,
        re.I,
    )
...
def analyze_security(repo_path, files):
    ...
    for file in files:
        ...
        if os.path.basename(normalized).lower() in {
            "security_analyzer.py",
            "secret_exposure_analyzer.py",
        }:
            continue
