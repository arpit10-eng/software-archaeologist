    ...
    if isinstance(value, dict):
        ...
        for key in (
            ...
            if number is not None:
                return max(
                    0.0,
                    min(number / 100.0 * maximum, maximum),
                )
...
def calculate_health_score(analysis=None, **kwargs):
    analysis = (
        dict(analysis or {})
        if isinstance(analysis or {}, dict)
        else {}
    )
    ...
    overall_score = (
        round((total_score / maximum_score) * 100.0, 2)
        if maximum_score
        else 0.0
    )
